...
The tidied time protocol is saved in `tsynth.RData`.

```{r save-tsynth}
save(tsynth,file="tsynth.RData")
```

This new protocol is applied to pre-calculated data. Instead of re-running `trxgarnish`, time protocol is stripped and re-merged, as the only time-specific value (quadrant pattern) is irrelevant to section 3. A section 3-specific data frame with synthetic time is saved in `sec3.RData`.

```{r sec3-maker}
load("garn.RData")
sec3range<-tsynth$time[tsynth$section==3]
timevars<-colnames(tsynth)[-which(colnames(tsynth)=="time")]

sec3<-list()
for (i in 1:length(garn)) {
  itrx<-garn[[i]]$trx
  itrx<-itrx[itrx$time %in% sec3range,]
